...
class potential():
    ...
    def potential_cell(self):
        ...
        dy = y_1D[1] - y_1D[0]
        z_1D = np.linspace(0, cell[2, 2], Potential.shape[2])
        dz = z_1D[1] - z_1D[0]

        return total_V, V1,cell

class extra_waves():

    def __init__(self,total_V,h,cell, Nz_max,dft, inputDict):
        self.wf=dft
        self.total_V=total_V
        self.h=h
        self.cell=cell
        self.Nz_max=Nz_max
        self.Simulator = inputDict
    ...
    def get_psi(self,i,ispin,ik):
        ...
        psi = self.wf.get_psi_rec(i, ispin, ik)
        psi_real = np.fft.ifftn(psi)

        psi_match = np.fft.ifft2(psi_real[:, :, self.Simulator['izend']])
        nrm_psi = 1e4
        psi_match_1 = psi_match / nrm_psi
        residual = Residual_extra(self.total_V[:, :, :self.Nz_max], self.h, self.Simulator['izend'], self.wf.eps[ik, ispin, i],
                                  psi_match_1)

        istart_list = residual.iso_contour(gk_1, gk_2, self.wf.k_vec[ik, :],
                                           self.Simulator['cutoff'],self.Simulator ['izstart_min'], self.Simulator['limit'])
...
class sx_waves_reader():
    ...
    def __init__(self,  working_directory,fname):
        self.wfile = h5py.File(fname)
        self.working_directory=working_directory
        self._eps = None
        ...
        self.eps = self.wfile['eps'][:].reshape(-1, self.n_spin, self.n_states)
...
class FIM_simulations():
    ...
    # }
    def __init__(self,dft,extra,total_V,inputDict):
    ...
    def sum_all_states(self,ik):
        ...
        self.Nx = self.total_V.shape[0]
        self.Ny = self.total_V.shape[1]
        for IE in self.Simulator['ionization_energies']:
        ...
        for i in range(0, self.wf.n_states):
            for ispin in range(self.wf.n_spin):
                ...
                if self.wf.eps[ik, ispin, i] < self.Simulator['E_fermi'] / self.HARTREE_TO_EV:
                    continue

                if self.wf.eps[ik, ispin, i] < self.Simulator['E_max'] / self.HARTREE_TO_EV:
                    continue
                psi_extra = self.get_psi
                for IE in self.Simulator['ionization_energies']:
...
#functions for FIM images and 1D scan
    def FIM_image(self,xp,yp,all_totals,filepath,kpoints,spin):
        ...
        self.spin=spin
        for ik in range(kpoints):
            for ispin in range(spin):
                with h5py.File(filepath) as handle:
        ...
        FIM_image_case =np.zeros([self.xp.shape[0],self.yp.shape[0]],dtype=np.complex128)
        ...
        for ix in range(xp.shape[0]):
             for iy in range(yp.shape[0]):
                x=xp[ix]
                y=yp[iy]
                phase_1=np.exp(-1j*(self.gk_1[:,0]*x+self.gk_1[:,1]*y))
                phase_2= np.exp(-1j*(self.gk_2[:,0]*x+self.gk_2[:,1]*y))
